  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (isOffline) {
        ...
        const response = await fetch(`/api/hotel/bill_order/zomato`);
        const zomatoResponse = await response.json();
        if (zomatoResponse.returncode === 200 && zomatoResponse.output.length > 0) {
          const { Categories, Menus } = zomatoResponse.output[0];
