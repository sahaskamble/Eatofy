...
export default function TableOrderPage() {
    ...
    try {
      ...
      if (isOffline) {
        data = await billsCrud.BillPayment({
          ...
          delivery_charges: paymentDetails.deliveryCharges
        });
      } else {
  ...
  const calculateTotal = () => {
    ...
    return cartTotal + existingTotal + paymentDetails.deliveryCharges;
  };
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (isOffline) {
        // Fetch menus
        menusData = await menusCrud.readDeliveryMenus();
        // Fetch categories
        categoryData = await menuCategoryCrud.readMenuCategories();
      } else {
        const response = await fetch(`/api/hotel/bill_order/delivery`);
        const deliveryResponse = await response.json();
        if (deliveryResponse.returncode === 200 && deliveryResponse.output.length > 0) {
          const { Categories, Menus } = deliveryResponse.output[0];
            ...
            <div className="flex-1 overflow-y-auto">
              <form className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={paymentDetails.deliveryCharges}
                    onChange={(e) => setPaymentDetails(prev => ({ ...prev, deliveryCharges: parseFloat(e.target.value) || 0 }))}
                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-shadow"
                ...
                {paymentDetails.discountPercentage > 0 && (
                  <div className="mt-2 p-3 text-base bg-gray-50 rounded-lg">
                    ...
                    <div className="flex justify-between font-semibold">
                      ...
                      <span className="text-green-600">₹{paymentDetails.deliveryCharges}</span>
                    </div>
                    <div className="flex justify-between font-semibold">
                      <span className="text-gray-800">Total Amount:</span>
                      <span className="text-green-600">₹{(calculateFinalAmount(paymentDetails.amount, paymentDetails.discountPercentage) + paymentDetails.deliveryCharges).toFixed(2)}</span>
                    </div>
                  </div>
                )}
              </form>
