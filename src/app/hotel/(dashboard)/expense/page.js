            ...
            <div className="fixed z-50 inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center">
                <div className="bg-white w-full max-w-lg rounded-lg shadow-lg p-6">
                    ...
                    <form onSubmit={handleSubmit}>
                        <input type="text" placeholder="Expense Name" value={formData.expense_name} onChange={(e) => setFormData({ ...formData, expense_name: e.target.value })} required className="border p-2 rounded mb-2 w-full" />
                        <input type="date" placeholder="Date" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} required className="border p-2 rounded mb-2 w-full" />
                        <input type="text" placeholder="Payable To" value={formData.payable_to} onChange={(e) => setFormData({ ...formData, payable_to: e.target.value })} required className="border p-2 rounded mb-2 w-full" />
                        <input type="number" placeholder="Amount Paid" value={formData.amount_paid} onChange={(e) => setFormData({ ...formData, amount_paid: Number(e.target.value) })} required className="border p-2 rounded mb-2 w-full" />
                        <input type="number" placeholder="Amount Payable" value={formData.amount_payable} onChange={(e) => setFormData({ ...formData, amount_payable: Number(e.target.value) })} required className="border p-2 rounded mb-2 w-full" />
                        <select value={formData.payment_status} onChange={(e) => setFormData({ ...formData, payment_status: e.target.value })} required className="border p-2 rounded mb-2 w-full">
                            <option value="">Select Payment Status</option>
                            <option value="Paid">Paid</option>
                            <option value="Pending">Pending</option>
                        </select>
                        <select value={formData.payment_mode} onChange={(e) => setFormData({ ...formData, payment_mode: e.target.value })} required className="border p-2 rounded mb-2 w-full">
            ...
            <div className="fixed z-50 inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center">
                <div className="bg-white w-full max-w-lg rounded-lg shadow-lg p-6">
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <button type="submit" className="bg-blue-500 text-white p-2 rounded">Edit Expense</button>
                    </form>
                </div>
            </div>
        );
    ...
    return (
        <div className="container mx-auto p-4 pt-6">
            <div className="flex justify-between mb-4">
                ...
                <button onClick={() => setIsOpenAddPopup(true)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Add Expense</button>
            </div>
