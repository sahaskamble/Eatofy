...
export default function AvailableStock() {
  ...
  const filteredStock = stock.filter((item) =>
    item.ItemId?.ItemName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    item.ItemId?.Category.CategoryName.toLowerCase().includes(searchQuery.toLowerCase())
  );
                    ...
                    >
                      <td className="py-3 px-4">
                        <span className="text-xs font-medium text-gray-600 bg-gray-100 px-2 py-1 rounded-full">
                          {item.ItemId?.CategoryId?.CategoryName}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <div className="flex items-center">
                          <div className="w-7 h-7 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-xs font-medium mr-2">
                            {item.ItemId?.ItemName.charAt(0)}
                          </div>
                          <span className="text-sm text-gray-700">{item.ItemId?.ItemName}</span>
                        </div>
                      </td>
