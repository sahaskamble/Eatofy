...
const Dashboard = () => {
  ...
  const [totalhotelsubs, setTotalHotelSubs] = useState(0);
  const [totalsubs, setTotalSubs] = useState(0);
  const select = useRef();
  const [totalhotelsearch, setTotalhotelsearch] = useState(0);
  const [totalhotelsubsearch, setTotalHotelSubsearch] = useState(0);
  const [totalsubsearch, setTotalSubsearch] = useState(0);
  ...
  const filterDataByDate = (data, filterType, customAmount) => {
    const today = new Date();

    if (filterType === '2days') {
      const startDate = new Date(today);
      startDate.setDate(today.getDate() - 1);
      return data.filter((item) => new Date(item.StartDate) >= startDate);
    }
