...
class HotelSubscriptionCrud extends BaseCrud {
  ...
  async getHotelsWithValidSubscription() {
    try {
      // ISO format "YYYY-MM-DD" matches how dates are stored in DB
      ...
      return {
        ...
        output: Array.from(validHotels.values()).map(item => ({
          ...item.hotel,
          _subscription: {
  ...
  async readSubscription(hotel_id) {
    try {
      const result = await this.readMany({ HotelId: hotel_id, isValid: true });
      return result;
    } catch (error) {
      return { returncode: 500, message: error.message, output: [] };
    }
  ...
  async updateSubscriptionPayment(data) {
    try {
      ...
      const result = await this.update(
        { _id: data.hotel_subscription_id },
        updateData,
        { new: true }
      );
      return result;
...
//         returncode: 200,
//         message: "Successfully fetched hotels with valid subscriptions",
//         // FIX: attach subscription info onto each hotel object so the frontend can use EndDate
//         output: Array.from(validHotels.values()).map(item => ({
//           ...item.hotel,
//           _subscription: {
//             EndDate: item.EndDate,
