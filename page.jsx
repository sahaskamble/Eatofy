...
export default function Menu() {
  ...
  // Initialize KOT number from sessionStorage
  const [kotNo, setKotNo] = useState(1);

  useEffect(() => {
    try {
      // Check if localStorage exists
      const storedKotNo = localStorage.getItem('kotNo');
      console.log('Stored KOT No:', storedKotNo);
      
      if (!storedKotNo) {
        console.log('No KOT number found, initializing to 1');
        localStorage.setItem('kotNo', '1');
        setKotNo(1);
      } else {
        console.log('Setting KOT number to:', parseInt(storedKotNo));
        setKotNo(parseInt(storedKotNo));
      }
    } catch (error) {
      console.error('Error handling KOT number:', error);
    }
  }, []);

  const incrementKotNo = () => {
    try {
      const currentKotNo = parseInt(localStorage.getItem('kotNo') || '1');
      console.log('Current KOT No:', currentKotNo);
      
      const nextKotNo = currentKotNo + 1;
      console.log('Next KOT No:', nextKotNo);
      
      localStorage.setItem('kotNo', nextKotNo.toString());
      setKotNo(nextKotNo);
    } catch (error) {
      console.error('Error incrementing KOT number:', error);
    }
  };
  ...
  const handlekotprintAndUpdatemenu = async () => {
    if (Cart.length === 0) {
      ...
      try {
        ...
        console.error('Error in handlekotprintAndUpdatemenu:', error);
      }
    }
  };
  ...
  const handleKotPrint = useReactToPrint({
    ...
    onBeforePrint: () => {
      console.log('Before print - Current KOT No:', kotNo);
      incrementKotNo();
    },
    onAfterPrint: () => {
      console.log('After print - New KOT No:', kotNo);
    }
  });

  const handlekotprintAndSavemenu = async () => {
    if (Cart.length === 0) {
      alert("Add some dish in the cart first");
    } else {
      try {
        ...
        console.error('Error in handlekotprintAndSavemenu:', error);
      }
    }
  };
          ...
          <div className="ml-[70px] flex px-0 bg-white">
                                  ...
                                  items.Status === "Cancelled" ? (
                                    <div key={index} className="flex p-2">
                                      <div className={`flex-1 inline-flex justify-start items-center gap-3 line-through text-red-500`}>
                                        <div
                                          className="w-[30px] h-[30px] p-2 bg-gray-400 inline-grid place-items-center rounded-md cursor-pointer"
                    ...
                    <div className="w-full p-2 flex flex-col gap-2 text-lg">
                      ...
                      <div className="w-full inline-flex justify-between items-center border-t border-zinc-500">
                        ...
                        <p className="font-semibold text-xl">
                          Rs.{totalAmt ? totalAmt : CalculateSubTotal() | 0}
                        </p>
                      </div>
                      ...
                      <div className="flex justify-start items-center gap-4 my-3 text-sm">
                        ...
                        <div
                          className={`p-3 rounded-md ${PaymentStatus === "Coins"
                            ? "bg-[#252836] border-2 font-semibold border-white text-white"
                            : "bg-[#252836] hover:font-semibold hover:text-white text-gray-300"
                            }`}
                          onClick={() => {
                            handlePaymentStatusClick("Coins");
            ...
            >
              <div className="flex flex-col justify-between mb-2 w-full">
                ...
                <span className="text-center">GST no: {GSTNO}</span>
